 ...
 * - POST /api/usuarios  → [Authorize(Roles = "ADMIN")] + EmpresaId == claim empresa_id
 ...
 */
import { isAdmin, isRrhh, isSuperadmin } from './roles.js'
...
export const API_ADMIN_ENDPOINTS = Object.freeze({
  ...
  crearUsuariosComoSuperadmin: false,
...
export function puedeCrearUsuarios(user, empresaId) {
  if (isRrhh(user) || isSuperadmin(user)) return false
